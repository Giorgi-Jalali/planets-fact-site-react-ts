...
export default function Earth() {
  ...
  let planetColor = miniData[2].color;
  let planetData = data[2];
...
const Photo = styled.div<{ internalPage: string }>`
  background-image: url(${(props) => props.internalPage === "structure" ? `${earthInternal}` : `${earth}`});
...
const PlanetGeology = styled.div<{
  ...
  background-image: url(${(props) => props.internalPage === "geology" ? `${earthGeology}` : "none"});
  background-repeat: no-repeat;
