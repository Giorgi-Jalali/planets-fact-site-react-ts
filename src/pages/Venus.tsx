...
export default function Venus() {
  ...
  let planetColor = miniData[1].color;
  let planetData = data[1];
...
const Photo = styled.div<{ internalPage: string }>`
  background-image: url(${(props) => props.internalPage === "structure" ? `${venusInternal}` : `${venus}`});
...
const PlanetGeology = styled.div<{
  ...
  background-image: url(${(props) => props.internalPage === "geology" ? `${venusGeology}` : "none"});
  background-repeat: no-repeat;
