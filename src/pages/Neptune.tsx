...
export default function Neptune() {
  ...
  let planetColor = miniData[7].color;
  let planetData = data[7];
...
const Photo = styled.div<{ internalPage: string }>`
  background-image: url(${(props) => props.internalPage === "structure" ? `${neptuneInternal}` : `${neptune}`});
...
const PlanetGeology = styled.div<{
  ...
  background-image: url(${(props) => props.internalPage === "geology" ? `${neptuneGeology}` : "none"});
  background-repeat: no-repeat;
