...
export default function Uranus() {
  ...
  let planetColor = miniData[6].color;
  let planetData = data[6];
...
const Photo = styled.div<{ internalPage: string }>`
  background-image: url(${(props) => props.internalPage === "structure" ? `${uranusInternal}` : `${uranus}`});
...
const PlanetGeology = styled.div<{
  ...
  background-image: url(${(props) => props.internalPage === "geology" ? `${uranusGeology}` : "none"});
  background-repeat: no-repeat;
