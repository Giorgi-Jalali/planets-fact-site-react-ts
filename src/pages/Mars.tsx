...
export default function Mars() {
  ...
  let planetColor = miniData[3].color;
  let planetData = data[3];
...
const Photo = styled.div<{ internalPage: string }>`
  background-image: url(${(props) => props.internalPage === "structure" ? `${marsInternal}` : `${mars}`});
...
const PlanetGeology = styled.div<{
  ...
  background-image: url(${(props) => props.internalPage === "geology" ? `${marsGeology}` : "none"});
  background-repeat: no-repeat;
